...
## Comparing today with past 10 `r format(today(), '%A')`s
...
```{r numbersHist, message=FALSE, warning=FALSE, error=FALSE,fig.height=2.8,fig.width=10}
Nyear <- Nhist %>%
...
```{r averages, message=FALSE, warning=FALSE, error=FALSE,fig.height=2.8,fig.width=10}
gridExtra::grid.arrange(
...
```{r mmsofar, message=FALSE, warning=FALSE, error=FALSE,fig.height=5.5,fig.width=9}
...
```{r delayToday,fig.height=4,fig.width=8,fig.align='center'}
Qday <- Qhist %>% 
    filter(date == Sys.Date())
 ggplot(Qday, aes(x = time)) +
    # geom_ribbon(aes(ymin = q5, ymax = q95), fill = colpal2$q90) + 
    # geom_ribbon(aes(ymin = q125, ymax = q875), fill = colpal2$q75) + 
    # geom_ribbon(aes(ymin = q25, ymax = q75), fill = colpal2$q50) + 
    geom_linerange(aes(ymin = q5, ymax = q95), color = colpal2$q90, lwd = 2) + 
    geom_linerange(aes(ymin = q125, ymax = q875), color = colpal2$q75, lwd = 2) + 
    geom_linerange(aes(ymin = q25, ymax = q75), color = colpal2$q50, lwd = 2) + 
    # geom_hline(yintercept = 0, color='white', lwd = 0.3) +
    xlab("Time (hour)") + ylab("Delay (minutes)") + 
    xlim(min(Qhist$time), max(Qhist$time)) + ylim(-20, 30)
```
