...
makePlots <- function(which = c("year", "6months", "3months", "month")) {
    which <- match.arg(which)

    date.filter <- as.numeric(strsplit(as.character(Sys.Date()), "-")[[1]])
    date.filter <- switch(which,
        "year" = c(ifelse(date.filter[2] < 12, date.filter[1] - 1, date.filter[1]), 
                   (date.filter[2]) %% 12 + 1, 1),
        "6months" = 
            c(ifelse(date.filter[2] > 6, date.filter[1], date.filter[1] - 1),
              (date.filter[2] - 6) %% 12 + 1, 1),
        "3months" = 
            c(ifelse(date.filter[2] > 3, date.filter[1], date.filter[1] - 1),
              (date.filter[2] - 3) %% 12 + 1, 1),
        "month" = c(date.filter[1], date.filter[2], 1)
        )
    date.filter <- 
        as.Date(sprintf("%s-%02d-%02d", date.filter[1], date.filter[2], date.filter[3]),
                oriign = "1970-01-01")
    ...
    jpeg(sprintf("~/Dropbox/gtfs/figs/history_n_%s.jpg", which), 
    ...
    jpeg(sprintf("~/Dropbox/gtfs/figs/history_q_%s.jpg", which), 
        width = 1920, height = 1080)
